...
namespace shyft {
    namespace core {
        /** \brief r_pm_gs_k namespace declares the needed r_pm_gs_k specific parts of a cell
        * that is:
        * -# the r_pm_gs_k parameter, state and response types
        ...
        */
            ...
            */
            struct all_response_collector {
                ...
                void collect(size_t idx, const response_t& response) {
                    ...
                    pe_output.set(idx, response.pm.et_ref);
                }
                //template<class R>
        ...
        template<>
        inline void cell<r_pm_gs_k::parameter_t, environment_t, r_pm_gs_k::state_t,
                         r_pm_gs_k::state_collector, r_pm_gs_k::all_response_collector>
            ::run(const timeaxis_t& time_axis, int start_step, int n_steps) {
            if (parameter.get() == nullptr)
                throw std::runtime_error("r_pm_gs_k::run with null parameter attempted");
            begin_run(time_axis,start_step,n_steps);
            r_pm_gs_k::run_r_pm_gs_k<direct_accessor, r_pm_gs_k::response_t>(
                geo,
                *parameter,
        ...
        template<>
        inline void cell<r_pm_gs_k::parameter_t, environment_t, r_pm_gs_k::state_t,
                         r_pm_gs_k::null_collector, r_pm_gs_k::discharge_collector>
            ::run(const timeaxis_t& time_axis, int start_step, int n_steps) {
            if (parameter.get() == nullptr)
                throw std::runtime_error("r_pm_gs_k::run with null parameter attempted");
            begin_run(time_axis, start_step, n_steps);
